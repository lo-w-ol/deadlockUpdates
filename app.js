(function() {
  ...
  function parsePost(post){ var normalized = normalizeSteamContent(post.contents || ''); var bullets = parseSectionsAndBullets(normalized); var changes = bullets.map(function(b,i){ return parseChangeLine(b.category,b.text,i); }); var categories = Array.from(new Set(changes.map(function(c){ return c.category; }))); var epoch = typeof post.date === 'number' ? post.date : Math.floor(Date.now()/1000); return { appid:APP_ID, gid:post.gid, title:post.title, url:post.url, date:new Date(epoch*1000).toISOString(), sourceType: post.sourceType || 'steam', isSubpatch: !!post.isSubpatch, categories:categories, changes:changes, rawText: normalized }; }
  ...
  function sourceLabel(p){ if (p.sourceType === 'forum-reply') return 'Forum Reply/Subpatch'; if (p.sourceType === 'forum-thread') return 'Forum Thread'; return 'Steam'; }
  function renderHome(){ el.detail.classList.add('hidden'); el.home.classList.remove('hidden'); el.home.innerHTML = state.filtered.map(function(p, idx){ var tops = summaryTop(p); var divider = idx < state.filtered.length - 1 ? '<div class="patch-divider" aria-hidden="true"></div>' : ''; return '<article class="card"><h3><a href="#/post/'+p.gid+'">'+escapeHtml(p.title)+'</a></h3><div class="muted">'+new Date(p.date).toLocaleString()+' • '+sourceLabel(p)+' • <a href="'+p.url+'" target="_blank" rel="noopener">Original URL</a></div><div class="chips">'+p.categories.map(function(c){ return chip(c); }).join('')+'</div><p class="muted">Parsed changes: '+p.changes.length+'</p><p class="muted">Top: '+escapeHtml(tops)+'</p>'+renderPatchCategories(p)+'</article>'+divider; }).join('') || '<div class="card">No posts match current filters.</div>'; }
  function renderPostDetail(gid){ var p = state.posts.find(function(x){ return x.gid===gid; }); if (!p){ location.hash='#/'; return; } el.home.classList.add('hidden'); el.detail.classList.remove('hidden'); var byCat = p.changes.reduce(function(m,c){ if (!m[c.category]) m[c.category] = []; m[c.category].push(c); return m; },{}); el.detail.innerHTML = '<h2>'+escapeHtml(p.title)+'</h2><p class="muted">'+new Date(p.date).toLocaleString()+' • '+sourceLabel(p)+' • <a href="'+p.url+'" target="_blank" rel="noopener">Open original post</a></p>'+'<button id="toggle-raw">Toggle raw text</button><pre id="raw" class="hidden">'+escapeHtml(p.rawText)+'</pre>'+Object.entries(byCat).map(function(entry){ var cat = entry[0], list = entry[1]; return '<section><h3>['+escapeHtml(cat)+']</h3>'+list.map(function(c){ return '<div class="change"><div>'+escapeHtml(c.raw)+'</div><div class="chips">'+c.tags.map(chip).join('')+chip(c.changeType,'tag-'+c.changeType)+'</div></div>'; }).join('')+'</section>'; }).join(''); document.getElementById('toggle-raw').onclick = function(){ document.getElementById('raw').classList.toggle('hidden'); }; }
