...
const MAX_ITEMS = 30;
const MAX_FORUM_PAGES = 5;
const MAX_FORUM_THREADS = 40;
const MAX_FORUM_REPLIES_PER_THREAD = 20;
...
async function fetchSteamFromOrigin(env) { const steamUrl = new URL('https://api.steampowered.com/ISteamNews/GetNewsForApp/v2/'); steamUrl.searchParams.set('appid', String(APP_ID)); steamUrl.searchParams.set('count', String(MAX_ITEMS)); steamUrl.searchParams.set('maxlength', '0'); steamUrl.searchParams.set('format', 'json'); steamUrl.searchParams.set('feeds', 'steam_community_announcements'); if (env.STEAM_API_KEY) steamUrl.searchParams.set('key', env.STEAM_API_KEY); const res = await fetch(steamUrl.toString(), { cf: { cacheTtl: 300, cacheEverything: false } }); if (!res.ok) throw new Error('Steam upstream ' + res.status); const json = await res.json(); const items=((json && json.appnews && json.appnews.newsitems) || []).map(function(item){ var body=String(item.contents||''); var canonicalTitle=canonicalizePatchText(item.title||''); var canonicalBody=canonicalizePatchText(body); var canonicalContentHash=simpleHash(canonicalTitle+'\n'+canonicalBody); return Object.assign({}, item, { sourceType:'steam', sourceId:String(item.gid||''), canonicalContentHash:canonicalContentHash, duplicateOf:null, isSubpatch:false }); }); return items; }

function decodeEntities(text){ return String(text||'').replace(/&nbsp;/gi,' ').replace(/&amp;/gi,'&').replace(/&lt;/gi,'<').replace(/&gt;/gi,'>').replace(/&quot;/gi,'"').replace(/&#39;/g,"'"); }
function stripMarkup(text){ return decodeEntities(String(text||'').replace(/<\s*br\s*\/?\s*>/gi,'\n').replace(/<\/p[^>]*>/gi,'\n').replace(/<[^>]+>/g,' ').replace(/\[\/?(?:p|list|b|i|u|h[1-6]|quote|code|url(?:=[^\]]+)?)\]/gi,' ').replace(/\[\*\]/gi,'\n- ')); }
function canonicalizePatchText(text){ return stripMarkup(text).toLowerCase().replace(/https?:\/\/\S+/g,' ').replace(/\b(?:click to expand|share:|similar threads|members online)\b/g,' ').replace(/[^a-z0-9\s]/g,' ').replace(/\s+/g,' ').trim(); }
function simpleHash(text){ var h=2166136261; var s=String(text||''); for (var i=0;i<s.length;i++){ h ^= s.charCodeAt(i); h += (h<<1)+(h<<4)+(h<<7)+(h<<8)+(h<<24);} return (h>>>0).toString(16); }
function likelyMeaningfulPatchText(text){ var t=canonicalizePatchText(text); if (!t || t.length<20) return false; return /(increased|decreased|reduced|fixed|added|removed|now|no longer|cooldown|damage|health|spirit|hero|item|ability|general)/.test(t); }
function parseForumThreadLinks(html, baseUrl){ var out=[]; var re=/<a[^>]+href=["']([^"']*\/threads\/[^"'#?]+)[^"']*["'][^>]*>/gi; var m; while((m=re.exec(html))!==null){ try{ out.push(new URL(m[1], baseUrl).toString()); }catch(_){}} return Array.from(new Set(out)); }
async function fetchForumIndexPages(){ var pages=[]; for (var i=1;i<=MAX_FORUM_PAGES;i++){ var u=i===1?'https://forums.playdeadlock.com/forums/changelog.10/':'https://forums.playdeadlock.com/forums/changelog.10/page-'+i; var r=await fetch(u,{cf:{cacheTtl:600,cacheEverything:false}}); if(!r.ok) break; pages.push(await r.text()); } return pages; }
async function fetchForumThread(threadUrl){ var res=await fetch(threadUrl,{cf:{cacheTtl:600,cacheEverything:false}}); if(!res.ok) return null; return await res.text(); }
function parseForumThreadPosts(html, threadUrl){ var title=((html.match(/<h1[^>]*class=["'][^"']*p-title-value[^"']*["'][^>]*>([\s\S]*?)<\/h1>/i)||[])[1]||'Forum Update').replace(/<[^>]+>/g,' ').trim(); var tid=((threadUrl.match(/\.([0-9]+)\/?$/)||[])[1]||''); var posts=[]; var re=/<article[^>]+data-content=["']post-([0-9]+)["'][^>]*>([\s\S]*?)<\/article>/gi; var m; while((m=re.exec(html))!==null){ var num=posts.length+1; var body=((m[2].match(/<div[^>]+class=["'][^"']*bbWrapper[^"']*["'][^>]*>([\s\S]*?)<\/div>/i)||[])[1]||''); var dt=((m[2].match(/<time[^>]+datetime=["']([^"']+)["']/i)||[])[1]||null); posts.push({ postNumber:num, postId:m[1], bodyHtml:body, bodyText:stripMarkup(body), date:dt?Math.floor(Date.parse(dt)/1000):null, url:threadUrl + '#post-' + m[1] }); } return { threadId:tid, threadUrl:threadUrl, title:title, posts:posts }; }
function normalizeForumPostToNewsItem(thread, post){ var sourceType=post.postNumber===1?'forum-thread':'forum-reply'; var gid=sourceType==='forum-thread'?'forum-thread:'+thread.threadId:'forum-reply:'+thread.threadId+':'+post.postNumber; var title=sourceType==='forum-thread'?thread.title:(thread.title+' (Reply #'+post.postNumber+')'); var canonicalTitle=canonicalizePatchText(title); var canonicalBody=canonicalizePatchText(post.bodyText); return { gid:gid, sourceType:sourceType, sourceId:gid, title:title, contents:post.bodyText, url:post.url, date:post.date, forumThreadUrl:thread.threadUrl, forumPostNumber:post.postNumber, forumThreadId:thread.threadId, canonicalContentHash:simpleHash(canonicalTitle+'\n'+canonicalBody), duplicateOf:null, isSubpatch:post.postNumber>1 }; }
async function fetchForumChangelogFromOrigin(){ var pages=await fetchForumIndexPages(); var threadUrls=[]; pages.forEach(function(h){ threadUrls=threadUrls.concat(parseForumThreadLinks(h,'https://forums.playdeadlock.com/')); }); threadUrls=Array.from(new Set(threadUrls)).slice(0,MAX_FORUM_THREADS); var items=[]; for (var i=0;i<threadUrls.length;i++){ var html=await fetchForumThread(threadUrls[i]); if(!html) continue; var thread=parseForumThreadPosts(html, threadUrls[i]); var posts=thread.posts.slice(0,1+MAX_FORUM_REPLIES_PER_THREAD); for (var j=0;j<posts.length;j++){ if (posts[j].postNumber>1 && !likelyMeaningfulPatchText(posts[j].bodyText)) continue; items.push(normalizeForumPostToNewsItem(thread, posts[j])); } } return items; }
function mergeAndDedupePatchItems(steamItems, forumItems){ var all=[].concat(steamItems||[], forumItems||[]); var byHash={}; var out=[]; all.sort(function(a,b){ var ap=a.sourceType==='steam'?3:(a.sourceType==='forum-reply'?2:1); var bp=b.sourceType==='steam'?3:(b.sourceType==='forum-reply'?2:1); return bp-ap; }); all.forEach(function(it){ var key=it.canonicalContentHash||simpleHash(canonicalizePatchText((it.title||'')+' '+(it.contents||''))); if (!byHash[key]) { byHash[key]=it.gid; out.push(it); } else { it.duplicateOf=byHash[key]; if (it.sourceType==='forum-reply' && it.isSubpatch) out.push(it); } }); out.sort(function(a,b){ return (b.date||0)-(a.date||0); }); return { items: out, dedupedCount: all.length-out.length }; }
...
async function refreshStoredNews(env, source) {
  const steamItems = await fetchSteamFromOrigin(env);
  const forumItems = await fetchForumChangelogFromOrigin(env);
  const merged = mergeAndDedupePatchItems(steamItems, forumItems);
  ...
  const meta = { lastRefreshedAt: now, source: source || 'manual', count: items.length, steamCount: steamItems.length, forumCount: forumItems.length, dedupedCount: merged.dedupedCount, hash: nextHash };
